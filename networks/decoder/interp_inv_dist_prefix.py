...
class InterpInvDistPrefixNet(torch.nn.Module):
    ...
    def __init__(self, latent_size, out_channels, K=16):
        ...
        self.fc_in = torch.nn.Conv1d(latent_size+3, latent_size, 1)
        mlp_layers = [torch.nn.Conv1d(latent_size, latent_size, 1) for _ in range(2)]
    ...
    def forward(self, data, spatial_only=False, spectral_only=False):
        ...
        x = (x * distances).sum(dim=3) / distances.sum(dim=3)
        pos = (pos * distances).sum(dim=3) / distances.sum(dim=3)
