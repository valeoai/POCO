...
class Convolution_ConvPoint(torch.nn.Module):
    ...
    def forward(self, x, pos, support_points, neighbors_indices):
        ...
        points = batch_gather(pos, dim=2, index=neighbors_indices).contiguous()
        input = batch_gather(x, dim=2, index=neighbors_indices).contiguous()
