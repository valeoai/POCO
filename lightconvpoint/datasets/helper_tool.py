...
class DataProcessing:
    ...
    def grid_sub_sampling(points, features=None, labels=None, grid_size=0.1, verbose=0):
        ...
        """
        grid_sampler = GridSampling(grid_size)
        if (features is None) and (labels is None):
            data = Data(
            pos = torch.tensor(points, dtype=torch.float32))
            data = grid_sampler(data)
            return (data.pos).numpy(), None, None
        elif labels is None:
            data = Data(
            x = torch.tensor(features, dtype=torch.float32),
            pos = torch.tensor(points, dtype=torch.float32))
            data = grid_sampler(data)
            return (data.pos).numpy(), (data.x).numpy(), None
        elif features is None:
            data = Data(
            y = torch.tensor(labels,dtype=torch.long),
            pos = torch.tensor(points, dtype=torch.float32))
            data = grid_sampler(data)
            return (data.pos).numpy(), None, (data.y).numpy().astype(np.int32)
        else:
            data = Data(
            x = (torch.tensor(features, dtype=torch.float32) if features is not None else None),
            y = (torch.tensor(labels,dtype=torch.long) if labels is not None else None),
            pos = (torch.tensor(points, dtype=torch.float32) if points is not None else None))
            data = grid_sampler(data)
            return (data.pos).numpy(), (data.x).numpy(), (data.y).numpy().astype(np.int32)
