...
std::vector<torch::Tensor> quantized_pick_knn(const torch::Tensor points, const size_t nqueries, const size_t K){
	...
	for(size_t b=0; b < B; b++){
		...
		// 		discrete_set.insert(v);
		// 	}
		// 	if(discrete_set.size() == nqueries) break;
		// 	if(discrete_set.size() < nqueries){
		// 		div_min = div_tmp;
		// 	}else{
		// 		div_max = div_tmp;
		// 	}
		// 	div_tmp = (div_max+div_min)/2;
		// 	size_prev = discrete_set.size();
		// }
		// // cout << nqueries << " - " <<discrete_set.size() << " - " <<  div_tmp << endl;

		// ofstream ofs("logs_vox3.txt", std::ios_base::app); // append instead of overwrite
		// ofs << nqueries << " " << div_tmp << endl; 
		// ofs.close();
